...
sma_21 <- lapply(stocks_cleaned$stocks, FUN=function(x){TTR::SMA(quantmod::Cl(x), n = 21)})

minervini <- Map(xts::merge.xts, stocks_cleaned$stocks, sma_200, sma_150, sma_50, min_250, max_250, sma_21)
columnames <- c("Close", "SMA_200", "SMA_150", "SMA_50", "Minimum_250", "Maximum_250", "SMA_21")
...
for (i in 1:length(ttt$`Anzahl Kriterien`)) {
    ...
    GD21 <-  SMA(na.locf(Cl(get(as.character(ttt$Ticker[i]), envir=stocks))), n = 21)
    ...
    chartSeries(get(as.character(ttt$Ticker[i]), envir=stocks), theme = chartTheme('white',up.col='gray',dn.col='red'),
                ...
                minor.ticks = FALSE,
                TA = c(addTA(GD50, on=1, col ="black"), addTA(GD150, on=1, col = "darkblue"), addTA(GD200, on=1, col = "red"), addTA(GD21, on=1, col = "green"))
                )
