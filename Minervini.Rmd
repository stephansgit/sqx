...
**!!! Durchgestrichene Kriterien sind noch nicht implementiert !!!**

1. Der Kurs ist über dem GD 150 und über dem GD 200
2. Der GD 150 ist über dem GD 200
3. ~~Der GD 200 bewegt sich seit über einem Monat aufwärts~~
...
min_200 <- lapply(stocks_cleaned$stocks, FUN=function(x){rollapply(x, width=250, FUN=min, na.rm=TRUE)})
max_200 <- lapply(stocks_cleaned$stocks, FUN=function(x){rollapply(x, width=250, FUN=max, na.rm=TRUE)})
sma_up <- NA
...
crits <- Map(xts::merge.xts, min_1, min_2, min_4, min_5)
columnames <- c("Krit_1", "Krit_2", "Krit_4", "Krit_5") 
...
ttt <- crits_res %>% dplyr::filter(Count >= 4)

```

Tabelle der Aktien, die alle 4 Kriterien erfüllen:
...
Charts der Aktien, die alle 4 Kriterien erfüllen:    
    


```{r charts1}

BRKB.EMA.20<- EMA(Cl(get(as.character(ttt$Ticker[i]), envir=stocks)), n=20)

for (i in 1:length(ttt$Count)) {
  
    GD50 <-  SMA(Cl(get(as.character(ttt$Ticker[i]), envir=stocks)), n = 50)
    ...
    chartSeries(get(as.character(ttt$Ticker[i]), envir=stocks), theme ="white",
                subset='last 12 months', 
                name=paste0(as.character(ttt$Ticker[i]), "(", ttt$Ticker[i], ")"),
                TA = c(addTA(GD50, on=1, col ="black"), addTA(GD150, on=1, col = "red"), addTA(GD200, on=1, col = "darkblue")))
