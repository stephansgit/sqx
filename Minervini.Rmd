...
for (i in 1:length(ttt$Count)) {
  try(
    chartSeries(get(as.character(ttt$Ticker[i]), envir=stocks), subset='last 12 months', name=paste0(as.character(ttt$Ticker[i]), "(", ttt$Ticker[i], ")") ) 
    )
    try(message(paste0(as.character(ttt$Ticker[i]), "(", ttt$Ticker[i], ")",
                 index(tail(get(as.character(ttt$Ticker[i]), envir=stocks),1))))
    )
}
```
