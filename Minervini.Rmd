...
ema_21 <- lapply(stocks_cleaned$stocks, FUN=function(x){TTR::EMA(quantmod::Cl(x), n = 21)})

minervini <- Map(xts::merge.xts, stocks_cleaned$stocks, sma_200, sma_150, sma_50, min_250, max_250, ema_21)
columnames <- c("Close", "SMA_200", "SMA_150", "SMA_50", "Minimum_250", "Maximum_250", "EMA_21")
...
min_8 <- lapply(minervini, minervini_8) #nicht Minervini, sondern Werners Anfrage

crits <- Map(xts::merge.xts, min_1, min_2, min_3, min_4, min_5, min_6, min_7, min_8)
columnames <- c("Krit_1", "Krit_2", "Krit_3", "Krit_4", "Krit_5", "Krit_6", "Krit_7", "Krit_8") 
...
ttt <- dplyr::arrange(ttt, desc(`Anzahl Kriterien`),   `Alter des Signals`)
```
...
Zusätzlich fügen wir noch einen EMA 21 als achtes Kriterium hinzu. 

```{r uebersicht}
hist(crits_res$Count, labels=TRUE, breaks = -1:8, 
     main = paste("Verteilung der erzielten Kriterien\n", "Anzahl Aktien:" ,length(stocks_cleaned$stocks)), 
     col=c(rep("grey",7), "red", "red"), xlab="Anzahl Kriterien erfüllt", ylab="Häufigkeit")


```

#### Tabelle der Aktien, die alle 7 Kriterien erfüllen    
(sortiert nach _acht_ oder _sieben_ Kriterien sowie dem Alter des Signals, d.h. vor wie vielen (Wochen-)Tagen das Signal erfolgte)
...
for (i in 1:length(ttt$`Anzahl Kriterien`)) {
    ...
    EMA21 <-  EMA(na.locf(Cl(get(as.character(ttt$Ticker[i]), envir=stocks))), n = 21)
    ...
    chartSeries(get(as.character(ttt$Ticker[i]), envir=stocks), theme = chartTheme('white',up.col='gray',dn.col='red'),
                ...
                minor.ticks = FALSE,
                TA = c(addTA(GD50, on=1, col ="darkgreen"), addTA(GD150, on=1, col = "darkblue"), addTA(GD200, on=1, col = "red"), addTA(EMA21, on=1, col = "black"))
                )
...
"Kriterien positiv" zählt die positiven Kriterien zusammen. Sind alle 7 Kriterien erfüllt, taucht die Aktie im Scanner auf. Sind acht Kriterien erfüllt, bedeutet das auch, dass die Aktie über dem EMA 21 liegt (und somit 'aufwärts trendet').   
