...
```{r plot_tbl, results='asis', include=FALSE}
recent_quotes <- data.frame(t(tail(round(de_vbt,2),5)))
recent_quotes$Ticker <- rownames(recent_quotes)
table <- merge(recent_quotes, fullnames)
names(table)[c(2:6)] <- as.character(index(tail(de_vbt,5)))
