...
last_stock <- rownames(last)[which(last>1)]
info <- if(is.null(last_stock)) {
  print("Keine Aktien hat ein Signal generiert")
  }   else { last_stock}
```

Die folgenden Aktien haben am vergangen Closing einen entspechende Wert gezeigt:  
`r info`  .

(_Leer_ bedeutet, dass kein Wert den Volumendurchschnitt 'durchbrochen' hat.)  

Die Tabelle zeigt die Werte für die letzten fünf Handelstage an; Klicken auf die Spaltenköpfe sortiert die Werte und man kann Werte über eins identifizieren.
...
- Füge weitere Akien hinzu - MDAX und SDAX(?)
- füge Grafiken hinzu.
