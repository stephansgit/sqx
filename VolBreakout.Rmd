...
dax_vbt <- dax_vol / (rollapply(dax_vol, width=lookback, FUN=mean, na.rm=T, align="right"))
...
```{r mosaicplot}
dax_signal <- dax_vbt["20140701::"]
dax_signal <- ifelse(dax_signal<trigger,FALSE,TRUE)
dax_signal <- xts2df(dax_signal)
dax_signal <- melt(dax_signal, id.vars="Date", variable.name="Stock", value.name="Signal")
ggplot(data=dax_signal) + geom_tile(aes(x=Date, y=Stock, fill=Signal)) + scale_x_bd(business.dates=yahoo('^GDAXI')) +  scale_fill_brewer(palette="Paired") + theme_minimal() + ggtitle("Mosaikplot der VB-Signale")# uses business days of DAX traded
```
