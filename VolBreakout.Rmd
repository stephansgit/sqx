...
- Datenquelle ist nun Frankfurter Börse, nicht mehr Xetra. Datenversorgung insb. kleinerer Werte scheint mit Frankfurter Werten besser zu sein.

__NEWS 20. April 2015__:  
...
ticker <- read.csv(file="tickerlist.csv")
#ticker <- ticker[1:100,]
#remove <- c("HLE.DE", "TLG.DE")
#de_stocks <- de_stocks[!de_stocks%in%remove]
de_stocks <- as.character(ticker[-1,1])
de_stocks <- gsub(".DE", ".F", de_stocks) # replace the Xetra-suffix with Frankfurt suffix
...
Eine Vielzahl deutscher Werte wird eingelesen, siehe Liste unter http://stooq.com/db/l/?g=29. Dennoch gibt es __nicht__ für alle entsprechende Volumendaten.  
Eingelesen werden gehandelte Volumen ~~auf Xetra~~ von der Börse Frankfurt; die Datenstabilität scheint hier höher zu sein als bspw. bei Xetra-Volumen.  .  
...
``` `r length(setdiff(de_stocks, ls(envir=stocks)))` ```  
