...
Die folgenden Aktien haben am vergangen Closing einen _VB_-Wert von __über__ `r trigger` bzw. __kleiner als__ -`r trigger` gezeigt (was einem Volumen über dem `r trigger`fachen Durchschnittsvolumen entspricht).

```{r table_vbts, results='asis'}
kable(merge(data.frame(Ticker=signals$signal_up), fullnames), caption='Werte mit Upmove heute ("weiße Kerze")')
print("")
kable(merge(data.frame(Ticker=signals$signal_dn), fullnames), caption='Werte mit Downmove heute ("schwarze Kerze")')
```
